...
{
    delegate SveltoDictionary<uint, T> ForceUnmanagedCast<T>(uint size) where T : IEntityComponent;
    ...
    static class ConvertStructToUnmanaged<T> where T : IEntityComponent
    {
        ...
        static class Trick
        {
            ...
            public static SveltoDictionary<uint, U> ForceUnmanaged<U>
                (uint size) where U : unmanaged, T, IEntityComponent
            {
                return new SveltoDictionary<uint, U>(size, new NativeStrategy<U>());
            }
        }
    }
    ...
    sealed class TypeSafeDictionary<TValue> : ITypeSafeDictionary<TValue> where TValue : struct, IEntityComponent
    {
       ...
       , ITypeSafeDictionary realDic, ExclusiveGroupStruct group, in PlatformProfiler profiler)
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public SveltoDictionary<uint, TValue>.FasterDictionaryKeyValueEnumerator GetEnumerator()
        {
       ...
       , in PlatformProfiler profiler, ExclusiveGroupStruct group)
        {
            foreach (var value in implementation)
                RemoveEntityComponentFromEngines(entityComponentEnginesDB, ref implementation.GetValueByRef(value.Key)
                                               , null, in profiler, new EGID(value.Key, group));
        }
